...
const NAV_ITEMS = [
  ...
  { label: "AI Studio", icon: "✨", href: "#" },
  { label: "Profile", icon: "👤", href: "/profile" },
  { label: "Country Intel", icon: "🌍", href: "#" },
  ...
  useEffect(() => {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
                (supabase.auth as any).getUser().then(({ data }: { data: { user: User | null } }) => {
                        ...
                        else { setUser(currentUser); loadStats(); }
                });
  }, [router]);
    ...
    return (
          <div className="min-h-screen flex" style={{ background: "#F8F8F6" }}>
            ...
            {sidebarOpen && (
                ...
                <aside className={`fixed lg:static inset-y-0 left-0 z-50 w-64 flex flex-col justify-between transition-transform lg:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`} style={{ background: "#0F1629" }}>
                        ...
                        <div className="relative flex flex-col h-full">
                                  <div className="px-5 pt-5 pb-4 border-b border-white/5">
                                              <Image src="/LOGO_PNG.png" alt="EuroCareer AI" width={140} height={32} className="h-7 w-auto" style={{ filter: "brightness(0) invert(1)" }} />
                                  </div>div>
                                  <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
                                    {NAV_ITEMS.map((item) => (
                          <Link key={item.label} href={item.href} className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors text-white/50 hover:text-white hover:bg-white/5">
                                          <span className="text-base">{item.icon}</span>span> {item.label}
                          </Link>Link>
                        ))}
                                  </nav>nav>
                          {/* AI Credits card */}
                  ...
                  {/* Top bar */}
                        <header className="bg-white border-b border-[#E2E1DC] px-6 py-4 flex items-center justify-between sticky top-0 z-30">
                                  ...
                                  <div className="flex items-center gap-3">
                                              ...
                                              <Button variant="outline" size="sm" onClick={handleSignOut} className="rounded-lg border-[#E2E1DC] text-sm font-medium hover:border-[#636DF5] hover:bg-[#EEEFFE] transition-all" style={{ color: "#3D4255" }}>
                                                            Sign out
                                              </Button>Button>
                                  </div>div>
                        </header>header>
            ...
            { label: "AI Gens", value: 3, icon: null, color: "#636DF5", star: true },
                          ...
                          {/* Quick actions */}
                            ...
                            <Link key={action.label} href={action.href} className="flex items-center gap-3 p-4 rounded-xl border border-[#E2E1DC] hover:border-[#636DF5] hover:bg-[#EEEFFE] transition-all group">
