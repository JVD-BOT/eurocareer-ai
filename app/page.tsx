...
const HOW_IT_WORKS = [
  { step: "01", icon: "ð¤", title: "Create your profile", desc: "Upload your CV once and you're set. EuroCareer AI uses it as the foundation for every country-specific adaptation." },
  { step: "02", icon: "ð", title: "Track your applications", desc: "See every application at a glance â track where you stand, from first look to final offer." },
  { step: "03", icon: "â¨", title: "Generate with AI", desc: "Pick a country, click adapt. In seconds your CV speaks the local language â formatted to the standards recruiters in that country actually expect." },
  ];

const FEATURES = [
  { emoji: "ð", title: "Application Tracker", desc: "See exactly where every application stands â at a glance. Filter by country or stage and never lose track of an opportunity.", tag: "Core" },
  { emoji: "ð", title: "AI CV Adapter", desc: "One click and your CV is restructured for the country you're targeting â format, length, and photo norms all handled.", tag: "AI" },
  { emoji: "ð", title: "Cover Letter Generator", desc: "Set your tone, get a personalised letter in seconds. Not happy? Regenerate with feedback until it's right.", tag: "AI" },
  { emoji: "ð", title: "Country Intelligence", desc: "Know exactly what recruiters in each country expect â photo rules, CV length, language conventions, and more.", tag: "Core" },
  ];

const BLOG_POSTS = [
  { tag: "CV Guide", flag: "ð©ðª", title: "How to write a CV for Germany in 2026", desc: "German CVs follow strict conventions around format, photo, and structure.", href: "/blog/german-cv-format" },
  { tag: "Job Search", flag: "ð³ð±", title: "Finding jobs in the Netherlands as a foreigner", desc: "The Dutch job market is open to internationals â here is how to stand out.", href: "/blog/netherlands-job-search" },
  { tag: "CV Guide", flag: "ðªðº", title: "EU vs US resume: key differences explained", desc: "Here is exactly what to change before you hit send on an EU application.", href: "/blog/eu-vs-us-resume" },
  ];

const FAQS = [
  { q: "Which countries are supported?", a: "Germany, Netherlands, France, Sweden, Denmark, UK, Spain, Ireland, Belgium, Austria, Switzerland, and Italy â 12 countries total." },
  { q: "How does the AI CV adapter work?", a: "Paste your existing CV, select the target country, and the AI rewrites it to match local hiring standards â format, length, photo norms, and more." },
  { q: "Do I need ChatGPT or any other AI subscription?", a: "No. The AI is built directly into EuroCareer AI. Everything runs within the app â no third-party subscriptions needed." },
  ...
  { q: "Can I cancel my Pro subscription at any time?", a: "Yes. Cancel anytime from your billing page â no questions asked, no lock-in." },
...
export default function LandingPage() {
  ...
  return (
      ...
      >
        {/* Background video â hidden on mobile (<768px) via .hero-video CSS */}
        <video
          src="/hero-loop.mp4"
          autoPlay
          muted
        ...
        {/* Dark overlay â rgba(15, 23, 42, 0.72) */}
        <div
          aria-hidden="true"
          style={{
            position: "absolute",
        ...
        {/* Hero content â sits above video + overlay */}
        <div
        ...
        >
          ...
          <p
            ...
            style={{ color: "#cbd5e1" }}
          >
            Every EU country has different CV rules, photo norms, and cover letter styles. EuroCareer AI adapts your application to match â 12 markets, in seconds.
          </p>
                      ...
                      <div className="max-w-5xl mx-auto px-6">
                        ...
                        <div key={f.title} ref={addReveal} className="reveal rounded-2xl border border-[#E2E1DC] bg-[#F8F8F6] p-6 hover:border-[#636DF5] hover:shadow-md transition-all hover:-translate-y-0.5" style={{ transitionDelay: `${i * 0.08}s` }}>
                                        <div className="flex items-start justify-between mb-3">
                                                          ...
                                                          <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${f.tag === "AI" ? "bg-[#EEEFFE] text-[#636DF5]" : "bg-[#F3F2EE] text-[#7A7F94]"}`}>{f.tag === "AI" ? "â¨ AI" : f.tag}</span>
                                        </div>
              ...
              <section id="pricing" className="py-20 max-w-5xl mx-auto px-6">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-w-3xl mx-auto">
                                <div ref={addReveal} className="reveal rounded-2xl border border-[#E2E1DC] bg-white p-8 space-y-4">
                                            ...
                                            <p className="text-4xl font-extrabold" style={{ fontFamily: "'Outfit', sans-serif" }}>â¬0</p>
                          ...
                          <li key={item} className="flex items-center gap-2"><span className="text-[#16A34A] font-bold">â</span>{item}</li>
                        ))}
                                            </ul>
                                ...
                                <div ref={addReveal} className="reveal rounded-2xl border-2 bg-white p-8 space-y-4 relative" style={{ borderColor: "#636DF5" }}>
                                            ...
                                            <p className="text-4xl font-extrabold" style={{ fontFamily: "'Outfit', sans-serif" }}>â¬9 <span className="text-base text-[#B0B4C5] font-normal">/ month</span></p>
                                            <ul className="space-y-2 text-sm text-[#3D4255]">
                          ...
                          <li key={item} className="flex items-center gap-2"><span className="font-bold" style={{ color: "#636DF5" }}>â</span>{item}</li>
                        ))}
                                            </ul>
              ...
              <section className="bg-white border-y border-[#E2E1DC] py-20">
                      <div className="max-w-5xl mx-auto px-6">
                                <div ref={addReveal} className="reveal flex items-center justify-between mb-10">
                                            ...
                                            <Link href="/blog" className="text-sm font-medium text-[#636DF5] hover:underline hidden sm:block">View all â</Link>
                                </div>
                        ...
                        <Link key={post.title} href={post.href} ref={addReveal} className="reveal group rounded-2xl border border-[#E2E1DC] bg-[#F8F8F6] p-6 hover:border-[#636DF5] hover:shadow-md transition-all hover:-translate-y-0.5 flex flex-col gap-3" style={{ transitionDelay: `${i * 0.08}s` }}>
                                        ...
                                        <span className="text-sm font-medium text-[#636DF5] group-hover:underline">Read guide â</span>
                        </Link>
                      ))}
                                </div>
                      </div>
                      ...
                      <div className="max-w-2xl mx-auto px-6">
                        ...
                        <details key={faq.q} ref={addReveal} className="reveal group rounded-xl border border-[#E2E1DC] bg-white px-6 py-4 cursor-pointer" style={{ transitionDelay: `${i * 0.05}s` }}>
                                        <summary className="font-medium text-[#0F1629] list-none flex items-center justify-between gap-4 text-sm">
                                          {faq.q}
                                                          <span className="text-[#B0B4C5] group-open:rotate-180 transition-transform shrink-0 text-xs">â¼</span>
                                        </summary>
          ...
          {/* ââ CTA ââ */}
              <section className="py-20 mx-4 sm:mx-8 mb-8 rounded-3xl text-white text-center relative overflow-hidden" style={{ background: "#0F1629" }}>
                      ...
                      <div className="relative max-w-xl mx-auto px-6">
                                ...
                                <p className="text-[#7A7F94] mb-8">Stop guessing what European recruiters want. Start free today â no credit card needed.</p>
                                <Link href="/auth/signup" className="inline-block bg-white text-[#0F1629] text-base font-semibold px-10 py-4 rounded-xl transition-all hover:-translate-y-0.5 hover:shadow-lg">Start free â</Link>
